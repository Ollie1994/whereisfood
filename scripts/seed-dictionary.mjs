...
// EVERY FAILURE MODE HERE HAS THE SAME CONSEQUENCE, which is why the parsing is
// stricter than a one-off script's usually is: anything that leaves `only` unset
// falls through to the unfiltered list and fires all sixteen requests at Nominatim.
// That is the exact opposite of what someone typing `--only` wants, it is rude to a
// free service, and it is silent. A first pass here rejected only the trailing
// `--only` and still let `--only=heden`, a bare `heden` and `--only ""` through —
// so the rule is now "recognise everything, reject anything else", rather than a
// list of the mistakes that had been thought of.
function parseArgs(argv) {
  ...
  for (let i = 0; i < argv.length; i++) {
    ...
    // Both spellings, because both are muscle memory and the `=` form silently
    // became an unknown token before.
    let value = null;
    if (arg === "--only") {
      value = argv[++i];
      ...
      // Anything unrecognised — including a bare `heden` with no flag — is a
      // usage error, not something to ignore. Ignoring it is what turned a typo
      // into sixteen requests.
      throw new Error(`unknown argument: ${arg}`);
    }

    // Covers the trailing `--only` (undefined), `--only --json` (the next flag
    // swallowed as a value), and `--only ""` (empty, falsy, filters to nothing).
...
async function main() {
  ...
  // Every stub below is printed with source: "nominatim" and verified: false,
  // because this script queries OSM and cannot know what a human has since
  // corrected. So a wholesale paste over dictionary.ts SILENTLY REVERTS exactly the
  // entries that were most expensive to get right — the hand-placed pins, which
  // types.ts says a re-run must not clobber. The output is per-entry on purpose;
  // this line is the warning that it is not a whole-file replacement.
  console.log("//");
  console.log("// ⚠ DO NOT paste wholesale over dictionary.ts. Every stub says source: \"nominatim\",");
  console.log("//   verified: false — so any entry a human corrected to \"manual\" or verified: true");
  console.log("//   would be silently reverted. Diff, then take only the entries you meant to update.");
